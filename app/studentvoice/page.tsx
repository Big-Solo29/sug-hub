...
function page() {
    ...
    return (
        <div>
            ...
            {/* Student voice modal */}
            {addMessage && (
                <section className=' fixed top-0 left-0 w-full h-screen bg-black/80 flex items-center justify-center'>
                    <div className='fixed top-0 left-0  w-full h-10 bg-white'></div>
                    <form
                        onSubmit={handleSubmit}
                        className="bg-white rounded-2xl sm:p-6 p-4 mb-6 sm:max-w-125 max-w-[90%] w-full"
                    >
            ...
            <section className='flex gap-8 '>
                ...
                <EventsCta />
                {addMessage && <button className="fixed sm:top-10 top-23 sm:right-8 right-1 w-10 h-10 rounded-full bg-[#1B7339] text-white shadow flex items-center justify-center text-xl" onClick={() => setAddMessage(!addMessage)}>
                    +
                </button>}
            </section>
        </div>
